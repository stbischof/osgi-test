...
public abstract class AbstractEventRecordingAssert<SELF extends AbstractEventRecordingAssert<SELF, ACTUAL>, ACTUAL extends EventRecording>
	...
	public DurationAssert hasDurationBetweenThat(int firstElementIndex, int secondElementIndex) {
		...
		hasEventsThat().hasSizeGreaterThanOrEqualTo(firstElementIndex + 1)
			.element(firstElementIndex)
			.isNotNull();

		hasEventsThat().hasSizeGreaterThanOrEqualTo(secondElementIndex + 1)
			.element(secondElementIndex)
			.isNotNull();

		TimedEvent<?> timedFirst = actual.timedEvents()
			.get(firstElementIndex);

		TimedEvent<?> timedSecond = actual.timedEvents()
			.get(secondElementIndex);
	...
	private DurationAssert createDurationAssert(TimedEvent<?> timedFirst, TimedEvent<?> timedSecond) {
		return new DurationAssert(Duration.between(timedFirst.getInstant(), timedSecond.getInstant()));
	}
