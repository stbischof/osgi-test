...
/**
 * {@link Condition}s on {@link BundleEvent}s, for use with AssertJ's
 * {@code is}/{@code has}/{@code are} methods and as predicates when waiting
 * for events.
 */
public final class BundleEventConditions {

	private BundleEventConditions() {}

	/**
	 * @param type a single {@link BundleEvent} type
	 * @return a condition matching events of exactly that type
	 */
	public static Condition<BundleEvent> type(int type) {
		if (!Bitmap.hasSingleBit(type) || type == 0) {
			throw new IllegalArgumentException(
				"Multiple bits set in type (" + type + ") - do you mean to use typeMaskedBy()?");
		}
		return new Condition<>(e -> e.getType() == type, "of type %s", BundleEventType.BITMAP.maskToString(type));
	}

	/**
	 * @param mask a mask of {@link BundleEvent} types
	 * @return a condition matching events of any of the types in the mask
	 */
	public static Condition<BundleEvent> typeMaskedBy(int mask) {
		return new Condition<>(e -> Bitmap.typeMatchesMask(e.getType(), mask), "of one of types [%s]",
			BundleEventType.BITMAP.maskToString(mask));
	}
	 ...
	 */
	public static Condition<BundleEvent> bundle(Bundle expected) {
		return new Condition<>(e -> e.getBundle() == expected, "for bundle %s", expected);
	}

	/**
	 * @param expected the bundle that must have caused the event
	 * @return a condition matching events whose origin is {@code expected}
	 */
	public static Condition<BundleEvent> origin(Bundle expected) {
		return new Condition<>(e -> e.getOrigin() == expected, "with origin %s", expected);
	}
}
