...
@FeatureLaunch(featureFile = "features/full-feature.json", framework = "org.eclipse.platform:org.eclipse.osgi:3.24.0", startupTimeoutSeconds = 30)
class ConfigAdminEquinoxTest extends ConfigAdminFeatureTestBase {
	...
	void systemBundleHasSymbolicName(FeatureLaunchCapture capture) {
		...
		BundleDTOAssert.assertThat(sys).hasSymbolicName("org.eclipse.osgi");
	}

	@Test
	void frameworkHasVendorProperty(FeatureLaunchCapture capture) {
		FrameworkDTO fwk = capture.frameworkDTO();
		FrameworkDTOAssert.assertThat(fwk).hasPropertiesThat().containsEntry("org.osgi.framework.vendor", "Eclipse");
	}
}
